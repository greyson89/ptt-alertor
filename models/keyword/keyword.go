...
func Subscribers(board string) (accounts []string) {
	rows, err := connections.DB().Query(
		"SELECT account FROM keyword_subscribers WHERE board = ?", board,
	)
	if err != nil {
...
func AddSubscriber(board, account string) error {
	_, err := connections.DB().Exec(
		"INSERT OR IGNORE INTO keyword_subscribers (board, account) VALUES (?, ?)", board, account,
	)
	if err != nil {
		log.WithField("runtime", myutil.BasicRuntimeInfo()).WithError(err).Error()
	}
...
func RemoveSubscriber(board, account string) error {
	_, err := connections.DB().Exec(
		"DELETE FROM keyword_subscribers WHERE board = ? AND account = ?", board, account,
	)
	if err != nil {
		log.WithField("runtime", myutil.BasicRuntimeInfo()).WithError(err).Error()
	}
...
func Destroy(board string) error {
	_, err := connections.DB().Exec("DELETE FROM keyword_subscribers WHERE board = ?", board)
	if err != nil {
		log.WithField("runtime", myutil.BasicRuntimeInfo()).WithError(err).Error()
	}
	return err
